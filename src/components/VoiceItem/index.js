...
export default class VoiceItem extends Component {
    ...
    _renderStatus = () => {
        ...
        if (data.status == VOICE_STATUS.UPLOADING) {
            return (
                <View className='row-start'>
        ...
        } else if (data.status == VOICE_STATUS.DONE) {
            return (
                <View className='row-start'>
        ...
        } else if (data.status == VOICE_STATUS.QUEUING) {
            return (
                <View className='row-start'>
        ...
        } else if (data.status == VOICE_STATUS.FAILED) {
            return (
                <View className='row-start'>
        ...
        } else if (data.status == VOICE_STATUS.PROCESSING) {
            return (
                <View className='row-start'>
                    <Text className='s12 blue' style={{ marginRight: 4 }}>{I18n.t('process_waiting')}</Text>
                    <Image source={require('~/src/image/dangcho.png')} style={{ width: 16, height: 16 }} />
                </View>
            )
        } else if (data.status == VOICE_STATUS.INITIAL) {
            return (
                <View className='row-start'>
            ...
            >
                <View className='row-start'>
                    ...
                    <View className='pv16 border-bottom flex' style={{ paddingRight: 14 }}>
                        ...
                        <View className='row-start'>
                            <Text className='s13 gray flex'>{moment(data.time * 1000).format(I18n.t('full_date_time_format'))}</Text>
                            {this._renderStatus()}
                        </View>
